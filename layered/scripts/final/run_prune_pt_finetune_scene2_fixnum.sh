  ...
  for i in "${!target_num[@]}"; do
    ...
    for prune_type in "${prune_types[@]}"; do
      ...
      while true; do
        ...
        if [[ -n $gpu_id ]]; then
          echo "GPU $gpu_id is available. Starting prune_finetune_fixnum.py with dataset '$arg', target_num '$target_num', prune_type '$prune_type', prune_decay '$prune_decay', and v_pow '$vp' on port $port"
        
          CUDA_VISIBLE_DEVICES=$gpu_id python prune_finetune_fixnum.py \
            -s "/scratch/jiasi_root/jiasi98/zxuechen/nerf360/$arg" \
            -m "/scratch/jiasi_root/jiasi98/zxuechen/OUTPUT/PATH/seperate/${arg}_${target_num}" \
            --eval \
            --port $port \
            ...
            --v_pow $vp > "logs_seperate/${arg}${target_num}_ply_prune2 .log" 2>&1
