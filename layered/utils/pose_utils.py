...
def generate_xyz_path(views):
# Read the input file and extract Head Position and Head Rotation
    def extract_trace(input_file, output_file):
        trace = []
        with open(input_file, 'r') as f:
            lines = f.readlines()
            for line in lines:
                match = re.search(r'Head Position: \(([^)]+)\), Head Rotation: \(([^)]+)\)', line)
                if match:
                    position = np.array(list(map(float, match.group(1).split(', '))))
                    rotation = np.array(list(map(float, match.group(2).split(', '))))
                    trace.append((position, rotation))
        
        # Save the extracted trace to a new file
        with open(output_file, 'w') as f:
            for position, rotation in trace:
                f.write(f"{position[0]}, {position[1]}, {position[2]}, {rotation[0]}, {rotation[1]}, {rotation[2]}\n")
        # ic(trace)
        return trace
    def euler_to_rotation_matrix(euler_angles):
        # Convert angles from degrees to radians
        euler_angles_rad = np.deg2rad(euler_angles)
        # Assuming euler_angles are in the order of x, y, z (roll, pitch, yaw)
        return R.from_euler('xyz', euler_angles_rad).as_matrix()
    def euler_to_quaternion(rx, ry, rz):
            # Convert degrees to radians
        rx_rad, ry_rad, rz_rad = np.radians([rx, ry, rz])
        r = R.from_euler('xyz', [rx_rad, ry_rad, rz_rad], degrees=False)
        q = r.as_quat()  # Returns in the order [qx, qy, qz, qw]
        return q[3], q[0], q[1], q[2]  # Reorder to [qw, qx, qy, qz]

        # Function to convert quaternion to rotation matrix
    def quaternion_to_rotation_matrix(qw, qx, qy, qz):
        r = R.from_quat([qx, qy, qz, qw])  # Note the order [qx, qy, qz, qw]
        return r.as_matrix()

        # Function to compute translation vector from rotation matrix and camera center
    def compute_translation_vector(R, camera_center):
        T = -np.dot(R, camera_center)
        return T

    def convert_trace_to_render_poses(trace):
        # Function to convert Euler angles (in degrees) to quaternion


        render_poses = []
        for position, euler_angles in trace:
            # qw, qx, qy, qz = euler_to_quaternion(*euler_angles)
            # R = quaternion_to_rotation_matrix(qw, qx, qy, qz)
            # T = compute_translation_vector(R, position)
            ...
            # Inverse to match the conventions used in render_poses
            render_pose = np.linalg.inv(transformation_matrix)
            # render_pose[:3, 1:3] *= -1  # Flip y and z axes to match conventions
            render_pose[:, 1:3] *= -1
                    # Adjust the position to correct the upside down issue
            render_pose[:3, 1] *= -1
            render_pose[:3, 2] *= -1
            render_poses.append(render_pose)

        return render_poses
    input_file = '/home/zxuechen/LightGaussian/trace/36fps_trace_cary_bicycle.txt'
    output_file = '/home/zxuechen/LightGaussian/trace/position_log_sophie_bike_output.txt'
    trace = extract_trace(input_file, output_file)
    render_poses = convert_trace_to_render_poses(trace)
    # ic(render_poses)
    return render_poses

def generate_rt_path(views,trace_path):
    ...
    for line in data:
        if line.strip():  # Check if the line is not empty
            ...
            current_poses.append(values)
            if len(current_poses) == 4:
                matrix = np.array(current_poses)
                render_poses.append(current_poses)
                current_poses = []
    return render_poses
