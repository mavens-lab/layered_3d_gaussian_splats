...
from gaussian_renderer import render,render_control_num
import torchvision
...
def render_xyz_trace(model_path, iteration, views, gaussians, pipeline, background,save_video):
    # render_path = os.path.join('/nfs/turbo/coe-mavens/zxuechen/LightGaussian/result', 'load_path_ours2', "ours_{}".format(iteration))
    render_path = os.path.join("/home/zxuechen/LightGaussian/result/concat_2", "concat")
    makedirs(render_path, exist_ok=True)
    view = views[0]
    # render_path_spiral
    # render_path_spherical
    # for idx, pose in enumerate(tqdm(generate_xyz_path(views), desc="Rendering progress")):
    #     index = int(180000/2160*idx*2.6)
    #     # view.world_view_transform = torch.tensor(getWorld2View2(pose[:3, :3].T, pose[:3, 3], view.trans, view.scale)).transpose(0, 1).cuda()
    #     # view.full_proj_transform = (view.world_view_transform.unsqueeze(0).bmm(view.projection_matrix.unsqueeze(0))).squeeze(0)
    #     # view.camera_center = view.world_view_transform.inverse()[3, :3]
    #     # rendering = render_control_num(index,view, gaussians, pipeline, background)["render"]
    #     # torchvision.utils.save_image(rendering, os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
