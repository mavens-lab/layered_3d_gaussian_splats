...
def render_control_num(
    ...
    screenspace_points = (
        torch.zeros_like(
            pc.get_xyz[0:index], dtype=pc.get_xyz.dtype, requires_grad=True, device="cuda"
        )
        + 0
    )
    try:
    ...
    means3D = pc.get_xyz[0:index]
    means2D = screenspace_points
    opacity = pc.get_opacity[0:index]
    ...
    if override_color is None:
        if pipe.convert_SHs_python:
            ...
            dir_pp = pc.get_xyz[0:index] - viewpoint_camera.camera_center.repeat(
                pc.get_features[0:index].shape[0], 1
            )
            dir_pp_normalized = dir_pp / dir_pp.norm(dim=1, keepdim=True)
            sh2rgb = eval_sh(pc.active_sh_degree, shs_view, dir_pp_normalized)
            colors_precomp = torch.clamp_min(sh2rgb + 0.5, 0.0)
        else:
            shs = pc.get_features[0:index]
    else:
        colors_precomp = override_color
