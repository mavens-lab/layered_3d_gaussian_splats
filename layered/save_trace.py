...
def render_video_save_trace(model_path, iteration, views, gaussians, pipeline, background):
...
def generate_trace(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, video: bool, circular:bool, radius: float, args):
    with torch.no_grad():
        ...
        if video:
            render_video_save_trace(dataset.model_path, scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background)
        #sample virtual view 
        if args.gaussians:
...
if __name__ == "__main__":
    ...
    generate_trace(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.video, args.circular, args.radius, args)