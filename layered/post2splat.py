...
def render_xyz_trace(model_path, iteration, views, gaussians, pipeline, background):
    render_path = os.path.join('/home/zxuechen/LightGaussian/OUTPUT', 'load_path', "ours_{}".format(iteration))
    makedirs(render_path, exist_ok=True)
    view = views[0]
    # render_path_spiral
    # render_path_spherical
    for idx, pose in enumerate(tqdm(generate_xyz_path(views,n_frames=600), desc="Rendering progress")):
        view.world_view_transform = torch.tensor(getWorld2View2(pose[:3, :3].T, pose[:3, 3], view.trans, view.scale)).transpose(0, 1).cuda()
        view.full_proj_transform = (view.world_view_transform.unsqueeze(0).bmm(view.projection_matrix.unsqueeze(0))).squeeze(0)
        view.camera_center = view.world_view_transform.inverse()[3, :3]
        rendering = render(view, gaussians, pipeline, background)["render"]
...
def load_xyz_trace(trace_path, iteration, views, gaussians, pipeline, background):
    directory_path = os.path.dirname(trace_path)
    view = views[0]
    # render_path_spiral
    # render_path_spherical
    with open(os.path.join(directory_path,'trace_splat_index.txt'),'w') as f: 
        for idx, pose in enumerate(tqdm(generate_xyz_path(views), desc="Rendering progress")):
            ...
            image, viewspace_point_tensor, visibility_filter, radii = (
                rendering["render"],
                rendering["viewspace_points"],
                rendering["visibility_filter"],
                rendering["radii"],
            )
            print('visibility_filter',visibility_filter)
            np.savetxt(f,visibility_filter.detach().cpu().numpy().astype(np.uint8).flatten(),delimiter=',',fmt='%f')
            f.write('\n')
...
def load_rt_trace(trace_path, iteration, views, gaussians, pipeline, background,save_video):
    directory_path = os.path.dirname(trace_path)
    view = views[0]
    with open(os.path.join(directory_path,'trace_splat_index.txt'),'w') as f: 
        for idx, pose in enumerate(tqdm(generate_rt_path(views,trace_path), desc="Rendering progress")):
            ...
            print('visibility_filter',visibility_filter)
            v_save = visibility_filter.detach().cpu().numpy().astype(np.uint8).flatten()
            # print(v_save)
            # np.savetxt(f,v_save,delimiter=',')
            # f.write('\n')
            v_save_str = ','.join(map(str, v_save))
            f.write(v_save_str + '\n'+ '\n')
            if idx == 5:
                break
        ...
        # )
        # print('gaussian_list',torch.max(gaussian_list))
        # print('imp_list',torch.max(imp_list))
        # if idx == 2: break
        #rendering = render(view, gaussians, pipeline, background)["render"]
        #torchvision.utils.save_image(rendering, os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
    # out_path = os.path.join(render_path,'video')
    ...
    with torch.no_grad():
        ...
        if args.trace:
            ...
            if args.trace_type == 'synthetic':
                load_rt_trace(args.trace_path, scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background,save_video=True)
        gaussian_list, imp_list = prune_list(gaussians, scene, pipeline, background)
