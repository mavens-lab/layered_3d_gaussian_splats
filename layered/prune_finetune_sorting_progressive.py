...
def training(
    ...
    elif args.start_pointcloud_sort:
        gaussians.load_ply_progressive(args.load_pointcloud_sort,args.start_pointcloud_sort,args.target_num_save,args.target_num_train)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # # freeze
        # pre_trained_size = int(args.small_num)
        gaussians._xyz.grad[:args.target_num_save] = 0.0
        gaussians._features_dc.grad[:args.target_num_save] = 0.0
        gaussians._features_rest.grad[:args.target_num_save] = 0.0
        gaussians._opacity.grad[:args.target_num_save] = 0.0
        gaussians._scaling.grad[:args.target_num_save] = 0.0
        gaussians._rotation.grad[:args.target_num_save] = 0.0
        viewspace_point_tensor.grad[:args.target_num_save] = 0.0
        ...
        with torch.no_grad():
            ...
            # ic("Final, number of gaussians: " + str(len(gaussians.get_xyz)))   
            if iteration < opt.iterations:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_iterations", nargs="+", type=int, default=[5,20,5_000,15_000,20_000,30_000,35_000,40_000,45_000,50_000]
    )
    parser.add_argument(
        "--save_iterations", nargs="+", type=int, default=[5_000,30_000,50_000]
    )
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument(
        "--checkpoint_iterations", nargs="+", type=int, default=[5_000,30_000,50_000]
    )

    parser.add_argument("--prune_iterations", nargs="+", type=int, default=[30_001])
    parser.add_argument("--start_checkpoint", type=str, default=None)
    parser.add_argument("--start_pointcloud_sort", type=str, default=None)
    parser.add_argument("--load_pointcloud_sort", type=str, default=None)
    #parser.add_argument("--prune_percent", type=float, default=0.1)
    parser.add_argument("--target_num_train", type=int, default=90_000)
    parser.add_argument("--target_num_save", type=int, default=45_000)
    parser.add_argument("--small_num", type=int, default=6_000)
