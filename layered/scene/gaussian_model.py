...
class GaussianModel:
    ...
    def load_ply_progressive(self, path_small,path_large,target_num_small,target_num_large):
        plydata_small = PlyData.read(path_small)
        plydata = PlyData.read(path_large)
        xyz = np.stack(
            (
                np.asarray(plydata.elements[0]["x"]),
                np.asarray(plydata.elements[0]["y"]),
        ...
        xyz_shape = xyz.shape[0]
        xyz_small = np.stack(
            (
                np.asarray(plydata_small.elements[0]["x"]),
                np.asarray(plydata_small.elements[0]["y"]),
                np.asarray(plydata_small.elements[0]["z"]),
            ),
            axis=1,
        )
        xyz_small_shape = xyz_small.shape[0]
        ic(xyz.shape)
        xyz = xyz[:target_num_large,:]
        xyz[:target_num_small,:] = xyz_small[:target_num_small,:]
        opacities = np.asarray(plydata.elements[0]["opacity"])[..., np.newaxis]
        opacities_small = np.asarray(plydata_small.elements[0]["opacity"])[..., np.newaxis]
        opacities = opacities[:target_num_large,:]
        opacities[:target_num_small,:] = opacities_small[:target_num_small,:]
        ic(opacities.shape)

        features_dc = np.zeros((xyz_shape, 3, 1))
        features_dc[:, 0, 0] = np.asarray(plydata.elements[0]["f_dc_0"])
        features_dc[:, 1, 0] = np.asarray(plydata.elements[0]["f_dc_1"])
        features_dc[:, 2, 0] = np.asarray(plydata.elements[0]["f_dc_2"])
        features_dc_small = np.zeros((xyz_small.shape[0], 3, 1))
        features_dc_small[:, 0, 0] = np.asarray(plydata_small.elements[0]["f_dc_0"])
        features_dc_small[:, 1, 0] = np.asarray(plydata_small.elements[0]["f_dc_1"])
        features_dc_small[:, 2, 0] = np.asarray(plydata_small.elements[0]["f_dc_2"])  
        features_dc = features_dc[:target_num_large,:]
        features_dc[:target_num_small,:] = features_dc_small[:target_num_small,:]
        extra_f_names = [
            p.name
            for p in plydata.elements[0].properties
            if p.name.startswith("f_rest_")
        ...
        # ic(extra_f_names)
        assert len(extra_f_names) == 3 * (self.max_sh_degree + 1) ** 2 - 3
        features_extra = np.zeros((xyz_shape, len(extra_f_names)))
        for idx, attr_name in enumerate(extra_f_names):
            features_extra[:, idx] = np.asarray(plydata.elements[0][attr_name])
        # Reshape (P,F*SH_coeffs) to (P, F, SH_coeffs except DC)
        features_extra = features_extra.reshape(
        ...
        features_extra_small = np.zeros((xyz_small_shape, len(extra_f_names)))
        for idx, attr_name in enumerate(extra_f_names):
            features_extra_small[:, idx] = np.asarray(plydata_small.elements[0][attr_name])
        # Reshape (P,F*SH_coeffs) to (P, F, SH_coeffs except DC)
        features_extra_small = features_extra_small.reshape(
            (features_extra_small.shape[0], 3, (self.max_sh_degree + 1) ** 2 - 1)
        )
        features_extra = features_extra[:target_num_large,:,:]
        features_extra[:target_num_small,:] = features_extra_small[:target_num_small,:,:]
        ...
        scale_names = sorted(scale_names, key=lambda x: int(x.split("_")[-1]))
        scales = np.zeros((xyz_shape, len(scale_names)))
        for idx, attr_name in enumerate(scale_names):
            scales[:, idx] = np.asarray(plydata.elements[0][attr_name])

        scales_small = np.zeros((xyz_small_shape, len(scale_names)))
        for idx, attr_name in enumerate(scale_names):
            scales_small[:, idx] = np.asarray(plydata_small.elements[0][attr_name])
        scales = scales[:target_num_large,:]
        scales[:target_num_small,:] = scales_small[:target_num_small,:]
        rot_names = [
            p.name for p in plydata.elements[0].properties if p.name.startswith("rot")
        ]
        rot_names = sorted(rot_names, key=lambda x: int(x.split("_")[-1]))
        rots = np.zeros((xyz_shape, len(rot_names)))
        for idx, attr_name in enumerate(rot_names):
            rots[:, idx] = np.asarray(plydata.elements[0][attr_name])
        rots_small = np.zeros((xyz_small_shape, len(rot_names)))
        for idx, attr_name in enumerate(rot_names):
            rots_small[:, idx] = np.asarray(plydata_small.elements[0][attr_name])
        rots = rots[:target_num_large,:]
        rots[:target_num_small,:] = rots_small[:target_num_small,:]          
        self._xyz = nn.Parameter(
            torch.tensor(xyz, dtype=torch.float, device="cuda").requires_grad_(True)
        )
        self._features_dc = nn.Parameter(
