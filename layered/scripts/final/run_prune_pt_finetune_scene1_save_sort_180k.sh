      ...
      while true; do
        ...
        if [[ -n $gpu_id ]]; then
          ...
          CUDA_VISIBLE_DEVICES=$gpu_id python prune_finetune_sorting_single.py \
            ...
            -m "/scratch/jiasi_root/jiasi98/zxuechen/OUTPUT/PATH/baseline/sort/${arg}_${prune_type}_ply_sort_all_from180" \
            --eval \
            --port $port \
            --start_pointcloud "/scratch/jiasi_root/jiasi98/zxuechen/OUTPUT/PATH/baseline/${arg}_180000/point_cloud/iteration_5000/point_cloud.ply" \
            --iteration 3 \
            --test_iterations 3 \
            --save_iterations 3 \
            --prune_iterations 2 \
            ...
            --v_pow $vp > "logs_sort/${arg}_${prune_type}_ply_sort_all_from180 .log" 2>&1
